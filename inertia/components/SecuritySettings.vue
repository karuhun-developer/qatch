  ...
  <div class="grid md:grid-cols-2 gap-6">
    ...
    <Card class="bg-card/60 backdrop-blur-xl border-muted/60 flex flex-col">
      <CardHeader>
        ...
        <CardDescription>
          Simpan string/secret HMAC Anda di sini. String ini akan digunakan sistem untuk memvalidasi
          *signature* pada *webhook* dari provider pembayaran.
        </CardDescription>
      </CardHeader>
      <CardContent class="flex-1">
        <form class="space-y-4" @submit.prevent="saveHmac">
          <div class="space-y-2">
            ...
            <p v-if="hmacForm.errors.hmacString" class="text-xs text-destructive mt-1">
              {{ hmacForm.errors.hmacString }}
            </p>
            <p
              v-if="hasHmac && !hmacForm.hmacString"
              class="text-xs text-green-500 mt-2 flex items-center gap-1"
            >
              <Check class="h-3 w-3" /> HMAC key sudah terkonfigurasi. Mengisi form ini akan
              menimpanya.
            </p>
          </div>
        </form>
      ...
      <CardFooter class="bg-muted/20 border-t border-muted/40 pt-4 mt-auto">
        <Button :disabled="hmacForm.processing || !hmacForm.hmacString" @click="saveHmac">
    ...
    <Card class="bg-card/60 backdrop-blur-xl border-muted/60 flex flex-col">
      <CardHeader>
        ...
        <CardDescription>
          Gunakan API Key ini untuk mengakses endpoint sistem. Kunci disimpan secara terenkripsi
          (two-way encryption) di database.
        </CardDescription>
      </CardHeader>
      <CardContent class="flex-1 flex flex-col justify-center">
        <div v-if="apiKey" class="space-y-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <Button variant="outline" size="icon" class="shrink-0" @click="copyApiKey">
              <Check v-if="isCopied" class="h-4 w-4 text-green-500" />
              <Copy v-else class="h-4 w-4" />
            </Button>
          </div>
          <p class="text-xs text-muted-foreground">
            Jaga kerahasiaan API Key Anda. Jangan pernah membagikannya ke publik.
          </p>
        </div>
      ...
      <CardFooter class="bg-muted/20 border-t border-muted/40 pt-4 mt-auto">
        ...
        >
          <RefreshCw
            v-if="apiKey"
            class="h-4 w-4"
            :class="{ 'animate-spin': apiForm.processing }"
          />
