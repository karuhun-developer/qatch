    ...
    >
      <DialogHeader class="p-6 pb-2">
        ...
        <DialogDescription class="text-muted-foreground mt-2">
          {{
            user
              ? 'Ubah data user di bawah ini. Kosongkan password jika tidak ingin mengubahnya.'
              : 'Masukkan data user baru yang ingin ditambahkan.'
          }}
        </DialogDescription>
      </DialogHeader>
      ...
      <form class="space-y-5 px-6 py-2" @submit.prevent="submit">
        <div class="space-y-2">
          ...
          />
          <p v-if="form.errors.fullName" class="text-xs text-destructive mt-1">
        ...
        <div class="space-y-2">
          ...
          <Select v-model="form.roleId" :disabled="form.processing" required>
            <SelectTrigger
              id="role"
              class="transition-all focus-visible:ring-primary/50 bg-background/50"
            >
          ...
          <p v-if="form.errors.roleId" class="text-xs text-destructive mt-1">
            {{ form.errors.roleId }}
          </p>
        </div>
