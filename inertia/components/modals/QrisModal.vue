  ...
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="sm:max-w-[425px] p-0 overflow-hidden border-muted/60 bg-background/95 backdrop-blur-xl">
      <DialogHeader class="p-6 pb-2">
        <DialogTitle class="text-xl font-semibold">{{ qrisItem ? 'Edit QRIS Statis' : 'Tambah QRIS Statis' }}</DialogTitle>
        <DialogDescription class="text-muted-foreground mt-2">
          {{ qrisItem ? 'Ubah data QRIS di bawah ini.' : 'Masukkan data QRIS baru yang ingin ditambahkan.' }}
        </DialogDescription>
      </DialogHeader>
      ...
      <form @submit.prevent="submit" class="space-y-5 px-6 py-2">
        ...
        <div class="space-y-2">
          <Label for="description" class="text-sm font-medium">Deskripsi <span class="text-muted-foreground font-normal">(Opsional)</span></Label>
          <Input 
            id="description" 
        ...
        <div class="space-y-2">
          <Label for="qrisFile" class="text-sm font-medium">Upload File QRIS <span class="text-muted-foreground font-normal">{{ qrisItem ? '(Opsional jika tidak diubah)' : '' }}</span></Label>
          <Input 
            id="qrisFile" 
            type="file"
          ...
          />
          <p v-if="form.errors.qris" class="text-xs text-destructive mt-1">{{ form.errors.qris }}</p>
        </div>

        <div v-if="form.qrisString" class="space-y-2">
          <Label class="text-sm font-medium text-emerald-500">✅ Payload QRIS Berhasil Diekstrak</Label>
          <Input 
            readonly 
            :modelValue="form.qrisString" 
            class="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 font-mono text-xs truncate"
          />
        </div>
        <div v-else-if="form.qris" class="space-y-2 p-3 bg-destructive/10 rounded-lg border border-destructive/20">
          <p class="text-xs font-medium text-destructive">Gagal mengekstrak QRIS dari gambar. Pilih gambar lain.</p>
        </div>
        ...
        <DialogFooter class="px-6 py-4 bg-muted/20 border-t border-muted/40 mt-6 -mx-6 sm:justify-end gap-2">
          ...
          <Button type="submit" :disabled="form.processing || !form.qrisString" class="shadow-md shadow-primary/20">
