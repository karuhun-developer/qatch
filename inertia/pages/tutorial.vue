...
import { ChevronRight, Menu, X, PlayCircle } from '@lucide/vue'
import { ref } from 'vue'
  ...
  <div class="max-w-screen-xl mx-auto flex relative">
    ...
    >
      <nav class="px-3 py-6 space-y-0.5">
        <p
          class="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-3 px-2 flex items-center"
        >
          <PlayCircle class="h-4 w-4 mr-1.5" /> Tutorial
        </p>
        <Link
          v-for="item in nav"
          :key="item.slug"
    ...
    <main class="flex-1 min-w-0 px-6 lg:px-12 py-10">
      ...
      <div class="flex items-center gap-1.5 text-sm text-muted-foreground mb-6">
        ...
        <Link href="/tutorial/setup" class="hover:text-foreground transition-colors">Tutorial</Link>
        <ChevronRight class="h-3.5 w-3.5" />
