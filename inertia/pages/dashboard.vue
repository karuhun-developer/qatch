...
<template>
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
    ...
    <Card class="md:col-span-3 bg-card/60 backdrop-blur-xl border-muted/60 relative overflow-hidden group">
      ...
      <CardContent class="relative z-10 h-[300px] w-full pt-4">
        <Line v-if="props.chartData.labels.length > 0" :data="chartDataConfig" :options="chartOptions" />
    ...
    <div class="md:col-span-1 space-y-4">
      <Card v-if="isSuperadmin" class="bg-gradient-to-br from-purple-500/10 to-transparent border-purple-500/20 hover:border-purple-500/40 transition-colors relative overflow-hidden">
        <div class="absolute -right-4 -top-4 bg-purple-500/10 w-24 h-24 rounded-full blur-2xl pointer-events-none"></div>
      ...
      <Card class="bg-gradient-to-br from-blue-500/10 to-transparent border-blue-500/20 hover:border-blue-500/40 transition-colors relative overflow-hidden">
        <div class="absolute -right-4 -top-4 bg-blue-500/10 w-24 h-24 rounded-full blur-2xl pointer-events-none"></div>
        <CardHeader class="flex flex-row items-center justify-between pb-2">
          <CardTitle class="text-sm font-medium text-muted-foreground">QRIS Statis Disimpan</CardTitle>
      ...
      <Card class="bg-gradient-to-br from-emerald-500/10 to-transparent border-emerald-500/20 hover:border-emerald-500/40 transition-colors relative overflow-hidden">
        <div class="absolute -right-4 -top-4 bg-emerald-500/10 w-24 h-24 rounded-full blur-2xl pointer-events-none"></div>
        <CardHeader class="flex flex-row items-center justify-between pb-2">
          <CardTitle class="text-sm font-medium text-muted-foreground">Total Transaksi Dinamis</CardTitle>
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
    ...
    <Card class="md:col-span-3 border-muted/60 flex flex-col">
      ...
      <CardContent class="flex-1 overflow-auto max-h-[400px]">
        <div class="space-y-4">
          <div v-for="tx in props.recentTransactions" :key="tx.id" class="flex items-center justify-between border-b pb-4 last:border-0 last:pb-0">
            <div class="flex items-center">
              ...
              <div class="space-y-1">
                ...
                <p class="text-xs text-muted-foreground">
                  <span v-if="isSuperadmin && tx.user" class="font-medium text-foreground">@{{ tx.user.fullName }} &bull;</span> 
            ...
            <div class="flex items-center space-x-4">
              <div class="text-right">
                <div class="font-bold text-sm">Rp {{ Number(tx.total).toLocaleString('id-ID') }}</div>
                <div class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold mt-1 uppercase"
                  :class="{
          ...
          <div v-if="props.recentTransactions.length === 0" class="text-center py-6 text-muted-foreground text-sm">
            Belum ada transaksi.
          </div>
        </div>
      </CardContent>
    ...
    <!-- System Health -->
    <div class="md:col-span-1 space-y-4">
      <Card class="bg-card/60 backdrop-blur-xl border-muted/60 relative overflow-hidden group hover:border-primary/50 transition-colors">
        <div class="absolute right-0 top-0 h-full w-1/2 bg-gradient-to-l from-emerald-500/10 to-transparent pointer-events-none"></div>
        <CardContent class="p-6">
          <div class="space-y-3 relative z-10">
            <div class="inline-flex items-center rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-500">
