...
import {
  generateDynamicQrisValidator,
  updateDynamicQrisStatusValidator,
} from '#validators/qris_transaction'
...
export default class DynamicQrisController {
  ...
  async update({ params, request, response, auth }: HttpContext) {
    ...
    if (payload.status === 'paid') {
      ...
      if (user.webhookUrl) {
        this.webhookService.dispatchTransactionPaid(
          user.webhookUrl,
          user.hmacKey,
          transaction.toJSON()
        )
  ...
  async callback({ request, response, auth }: HttpContext) {
    ...
    if (user.webhookTitleWildcard) {
      ...
      const wildcards = user.webhookTitleWildcard
        .split(',')
    ...
    if (user.webhookTextWildcard) {
      ...
      const wildcards = user.webhookTextWildcard
        .split(',')
        .map((w: string) => w.trim().toLowerCase())
        .filter(Boolean)
    ...
    const numbersInText = [...payload.notification_text.matchAll(/[\d.,]+/g)]
      .map((m) => Number.parseInt(m[0].replace(/[.,]/g, ''), 10))
      .filter((n) => !isNaN(n))
